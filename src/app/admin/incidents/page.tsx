...
export default function AdminIncidentsPage() {
  ...
  return (
    <div className='w-full p-4 md:p-6 space-y-6'>
      ...
      {/* SECTION 1: DETAILED ACTIVE DEVICES STREAM */}
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          <Cpu className="h-4 w-4 text-slate-700" />
          <h2 className="text-sm font-semibold text-slate-800">Daftar Hardware Terpantau ({deviceSummaryList.length})</h2>
        </div>
        
        {deviceSummaryList.length === 0 && !loading ? (
          <div className="text-xs text-muted-foreground bg-slate-50 p-4 rounded-xl border border-dashed text-center">
            Belum ada hardware IoT yang mengirimkan sinyal log ke Firestore.
          </div>
        ) : (
          <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 w-full">
            {deviceSummaryList.map((device) => {
              const statusInfo = getForceStatus(device.accelerometerForce);
              return (
                <Card key={device.deviceSn} className="bg-white shadow-sm border border-slate-200/70 rounded-xl overflow-hidden hover:shadow-md transition-all">
                  <div className="p-4 space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="font-mono font-bold text-xs text-primary truncate max-w-[110px]">
                          {device.deviceSn}
                        </span>
                      </div>
                      <Badge 
                        variant="outline"
                        className={
                          device.latestType?.toUpperCase() === 'JATUH'
                            ? 'bg-red-50 text-red-700 border-red-200 text-[10px] px-2 py-0'
                            : device.latestType?.toUpperCase() === 'WASPADA'
                            ? 'bg-amber-50 text-amber-700 border-amber-200 text-[10px] px-2 py-0'
                            : 'bg-emerald-50 text-emerald-700 border-emerald-200 text-[10px] px-2 py-0'
                        }
                      >
                        {device.latestType.toUpperCase()}
                      </Badge>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-1 border-t border-slate-100">
                      <div>
                        <p className="text-[9px] text-muted-foreground uppercase font-medium">Total Isyarat</p>
                        <p className="text-sm font-bold text-slate-800">{device.totalIncidents} <span className="text-[10px] font-normal text-muted-foreground">Log</span></p>
                      </div>
                      <div>
                        <p className="text-[9px] text-muted-foreground uppercase font-medium">G-Force Terakhir</p>
                        <p className="text-sm font-semibold font-mono text-slate-700">
                          {device.accelerometerForce ? `${Number(device.accelerometerForce).toFixed(2)}G` : '-'}
                        </p>
                      </div>
                    </div>

                    {/* Footer Info Perangkat */}
                    <div className="space-y-1.5 pt-1 text-[11px] text-slate-500">
                      <div className="flex items-center gap-1 truncate">
                        <Clock className="h-3 w-3 text-slate-400 shrink-0" />
                        <span className="truncate">Last: {device.latestDate}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <LocationCell lat={device.latitude} lng={device.longitude} />
                      </div>
                    </div>
                  </div>
                </Card>
