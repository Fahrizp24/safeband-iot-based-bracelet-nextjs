...
export default function RegisterPage() {
  ...
  const handleGoogleLogin = () => {
    if (!deviceSn) {
      toast.error('Silakan isi Serial Number Device (ESP32) terlebih dahulu');
      return;
    }
    document.cookie = `pending_device_sn=${encodeURIComponent(deviceSn)}; path=/; max-age=300`;
  ...
  return (
    <div className='flex min-h-screen font-sans antialiased bg-white'>
      ...
      <div className='relative hidden w-1/2 flex-col items-center justify-center bg-slate-900 overflow-hidden lg:flex'>
        ...
        <div className="relative z-10 flex flex-col items-center w-full px-12">
           ...
           <div className="mt-12 text-center">
             <h2 className="text-3xl font-bold text-white tracking-tight">Menjaga Mereka yang Berharga</h2>
             <p className="mt-4 text-slate-400 text-lg max-w-md mx-auto">
               Sistem pintar wearable pertama yang siap mendeteksi bahaya dan mencegah keterlambatan pertolongan.
             </p>
           </div>
        </div>

        {/* Subtle branding at bottom left */}
        <div className="absolute flex bottom-6 left-8 items-center gap-2 opacity-60">
      ...
      <div className='flex w-full flex-col justify-center px-4 py-8 sm:px-6 lg:w-1/2 lg:px-20 xl:px-32 bg-white'>
        <div className="mx-auto w-full max-w-md">
          {/* Logo only for mobile, visible on desktop as nice touch */}
          <Link href="/" className="mb-6 block max-w-fit hover:opacity-80 transition-opacity">
            <img 
          ...
          <h1 className='mt-4 text-3xl font-extrabold tracking-tight text-slate-900'>Buat Akun Baru</h1>
          <p className='mt-2 text-sm text-slate-500'>
            Daftarkan diri Anda untuk mulai menghubungkan perangkat SafeBand.
          </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className='space-y-2'>
                ...
                <p className='text-xs text-slate-500'>
                  Wajib mengisi SN di bagian belakang / layar perangkat Anda.
                </p>
              </div>

              <Button type='submit' className='w-full mt-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl h-12 text-base font-semibold shadow-md active:scale-[0.98] transition-transform' disabled={loading}>
                {loading && <Icons.spinner className='mr-2 h-4 w-4 animate-spin' />}
                Daftar & Hubungkan Perangkat
              </Button>
            </form>
            ...
            <div className="mt-8">
              <div className="relative">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="bg-white px-4 text-slate-500 font-medium">Atau lanjutkan dengan</span>
                </div>
              </div>
